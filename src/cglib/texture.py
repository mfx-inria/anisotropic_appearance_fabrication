...
from .type import int_


class ImageWrap(IntEnum):
    """
    Enumeration of the types of image texture wrap.

    Attributes
    ----------
    REPEAT
        Image texture coordinates are repeated when they are out of bounds.
    ZERO
        The evaluation of the texture returns zero when the image texture
        coordinates are out of bounds.
    CLAMP
        Image texture coordinates are clamped when they are out of bounds.
    """
    REPEAT = 0
    ZERO = 1
    CLAMP = 2
...
class InterpolationType(IntEnum):
    """
    Enumeration of the types of interpolation of pixel data.

    Attributes
    ----------
    NEAREST
        Take the nearest value from the evaluation point.
    BILINEAR
        Use bilinear interpolation to interpolate the values between the four
        nearest values.
    """
    NEAREST = 0
    BILINEAR = 0
...
def image_png_from_hamiltonian_format(
        filename_in: str,
        filename_lines: str,
        filename_mask: str) -> None:
    """Convert a texture from Hamiltonian cycle code to pngs.

    Create a png image and mask from a direction field exported from the
    Hamiltonian cycle code:
    https://github.com/mfx-inria/controllable-space-filling-curve.

    The images are saved on the disk with the png format. Each pixel has one
    channel and 8 bits per pixel.
    """
    with open(filename_in, 'r', encoding="utf-8") as f:
        first_line = f.readline().split()
        width = int(first_line[0])
        height = int(first_line[1])
        dir_count = width * height
        data = np.zeros((dir_count, 2))
        for i in range(dir_count):
            vec2_str = f.readline().split()
            data[i, 0] = float(vec2_str[0])
            data[i, 1] = float(vec2_str[1])
    # Vectors are in "line space", where original directions' angle had been
    # multiplied by two. In addition, the origin is located at the top-left
    # corner, implying a y
    mask = np.where(np.all(np.equal(data, 0.), axis=1), 1., 0.)
    mask = mask.reshape((height, width))
    angles_line_space = np.vectorize(
        np.arctan2, signature='(),()->()')(-data[:, 1], data[:, 0])
    angles = angles_line_space * 0.5
    angles_normalized = (angles + np.pi*0.5) / np.pi
    angles_normalized = angles_normalized.reshape((height, width))
    angles_normalized_uint8 = (angles_normalized * 255).astype(np.uint8)
    mask_uint8 = (mask * 255).astype(np.uint8)
    im = PILImage.fromarray(angles_normalized_uint8, mode="L")
    im_mask = PILImage.fromarray(mask_uint8, mode="L")
    im.save(filename_lines)
    im_mask.save(filename_mask)


def image_hamiltonian_format_from_png(
        filename_lines: str,
        filename_mask: str,
        filename_out: str) -> None:
    """Convert a texture to the format used in the Hamiltonian cycle code.

    Create a text file that can be read by the Hamiltonian cycle code
    (https://github.com/mfx-inria/controllable-space-filling-curve) from png
    images and mask.

    The images must be saved on the disk with the png format. Each pixel has
    one channel and 8 bits per pixel.
    """
    im = PILImage.open(filename_lines)
    im_mask = PILImage.open(filename_mask)
    angles_0_255 = np.array(im).astype(np.float32)
    mask_0_255 = np.array(im_mask).astype(np.float32)
    angles_normalized = angles_0_255 / 255.
    mask_normalized = mask_0_255 / 255.
    angles = angles_normalized * np.pi - np.pi*0.5
    angles_line_space = angles * 2.
    data_x = np.cos(angles_line_space)
    data_y = -np.sin(angles_line_space)
    angles = np.concatenate(
        (data_x.reshape(-1, 1), data_y.reshape(-1, 1)), axis=1)
    mask = mask_normalized.ravel()
    angles = np.where(mask.reshape(-1, 1), 0., angles)

    np.savetxt(filename_out, angles, fmt='%.6f')


def image_create_from_png_1channel_8bits(
        filename: str,
        wrap_mode: ImageWrap,
        device: xla_client.Device) -> Image:
    """Create an image texture from a grayscale png.

    Assume image with one channel per pixel and 8 bits per channel.
...
def image_eval_from_discrete_coordinates(s: jnp.ndarray, tex: Image) -> Any:
    ...
    ----------
    s : ndarray
        The image coordinate in discretized space, i.e., the texel coordinate.
    tex : Image
        The image texture.
    ...
    -------
    Any
        The data in the texture element (texel).
    """
    shape_xy = jnp.array(tex.data.shape[::-1])
    wrap_mode = tex.wrap_mode
    out_of_bound = jnp.less(s, jnp.zeros_like(s))
    out_of_bound = jnp.logical_or(out_of_bound, jnp.greater_equal(s, shape_xy))
    zero_cond = jnp.logical_and(
        jnp.equal(wrap_mode, ImageWrap.ZERO), jnp.any(out_of_bound))

    s_mod = jnp.mod(s, shape_xy)
    s_clamped = cgm.clamp(s, jnp.zeros_like(shape_xy), shape_xy)
    s = jnp.where(jnp.equal(wrap_mode, ImageWrap.REPEAT), s_mod, s)
    s = jnp.where(jnp.equal(wrap_mode, ImageWrap.CLAMP), s_clamped, s)
    tex_val = tex.data[s[1], s[0]]
    tex_val = jnp.where(zero_cond, jnp.zeros_like(tex_val), tex_val)
    return tex_val


def image_eval_linear(s: jnp.ndarray, tex: Image):
    """Evaluate the image texture with linear interpolation.

    Use a triangle filter to interpolate between the four texels that surround
    the sample point. It is also called linear interpolation.

    Parameters
    ----------
    s : ndarray
        Continuous texture coordinates in $[0, 1]^2$.
    tex : Image
        The image texture.
    ...
    -------
    Any
        The interpolated data at texture position.
    """
    # Scale the coordinates by the texture resolution in each direction,
    # turning them into continuous texel coordinates
    shape_xy = jnp.array(tex.data.shape[::-1])
    s_continuous = s * shape_xy - 0.5
    s_discrete = jnp.floor(s_continuous)
    ds = s_continuous - s_discrete
    s_discrete = s_discrete.astype(int_)

    v00 = image_eval_from_discrete_coordinates(s_discrete, tex)
    v01 = image_eval_from_discrete_coordinates(s_discrete + jnp.array([0, 1]), tex)
    v10 = image_eval_from_discrete_coordinates(s_discrete + jnp.array([1, 0]), tex)
    v11 = image_eval_from_discrete_coordinates(s_discrete + jnp.array([1, 1]), tex)

    return (1. - ds[0]) * (1. - ds[1]) * v00 +\
           (1. - ds[0]) * ds[1] * v01 +\
        ds[0] * (1. - ds[1]) * v10 +\
        ds[0] * ds[1] * v11


def image_eval_nearest(s: jnp.ndarray, tex: Image):
    """Evaluate the image texture with nearest interpolation.
    ...
    -------
    Any
        The nearest data from the continuous texture coordinates.

    Given real texture coordinates in $[0, 1]^2$, give the nearest value to the
    sample point.
    """
    # Scale the coordinates by the texture resolution in each direction,
    # turning them into continuous texel coordinates
    ...
    s_discrete = jnp.floor(s_continuous)
    s_discrete = s_discrete.astype(int_)
    return image_eval_from_discrete_coordinates(s_discrete, tex)


def image_eval(s: jnp.ndarray, tex: Image, interpolation: InterpolationType):
    if interpolation == InterpolationType.NEAREST:
        return image_eval_nearest(s, tex)
    if interpolation == InterpolationType.BILINEAR:
        return image_eval_linear(s, tex)


def cm_twilight_from_float_normalized(u: float) -> jnp.ndarray:
    """Map a normalized scalar in [0, 1) to color in the twilight colormap.
    ...
    -------
    ndarray
        A RGB color represented as a JAX ndarray with shape (3,).
    """
    discrete_coord = jnp.floor(u * cm_twilight_data.shape[0] + 0.5).astype(int)
    discrete_coord = jnp.maximum(discrete_coord, 0)
    discrete_coord = jnp.minimum(discrete_coord, cm_twilight_data.shape[0]-1)
    return cm_twilight_data[discrete_coord]


def twilight_from_png_1channel_8bits(
        image_filename_in: str,
        image_filename_out: str,
        device: xla_client.Device) -> None:
    """Convert an image texture to twilight from a grayscale image.

    The output image texture is saved in RGB mode. Twilight refers to the
    twilight color map: https://github.com/bastibe/twilight.

    Parameters
    ----------
    image_filename_in : str
        The filename of the input image. It must be a png image with one
        channel per pixel and 8 bits per channel.
    image_filename_out : str
        The filename of the output image.
    device : Device
        Specify the device the computation will run on. Available devices can
        be retrieved via `jax.devices()`.

    Returns
    -------
    None
    """
    tex: Image = image_create_from_png_1channel_8bits(
        image_filename_in, ImageWrap.CLAMP, device)

    tex_twilight = jax.vmap(
        cm_twilight_from_float_normalized)(tex.data.ravel())
    tex_twilight = tex_twilight.reshape(
        (tex.data.shape[0], tex.data.shape[1], -1))
    tex_twilight_uint8 = (tex_twilight * 255).astype(jnp.uint8)
    im = PILImage.fromarray(jax.device_get(tex_twilight_uint8), mode="RGB")
    im.save(image_filename_out)


# Twilight color map data
# Source: https://github.com/bastibe/twilight
# File: twilight.py
cm_twilight_data = jnp.array([
